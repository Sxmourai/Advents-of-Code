...
def predict_next(previous): # Declares function to predict next value
    ...
    for i,step in reversed(list(enumerate(previous))): # Iterate over our step in reversed order
        spe_num = step[0] # Gets the next number to add to our predicted value
        # print(step, n, spe_num)
        n=spe_num-n # Adds it
    # print(n)
